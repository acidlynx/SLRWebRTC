...
#import "sdk/objc/components/audio/SLRRTCAudioSession+Private.h"
#import "sdk/objc/components/audio/SLRRTCAudioSession.h"
#import "sdk/objc/components/audio/SLRRTCAudioSessionConfiguration.h"
...
AudioDeviceIOS::AudioDeviceIOS()
  ...
  audio_session_observer_ = [[SLRRTCAudioSessionDelegateAdapter alloc] initWithObserver:this];
}
...
AudioDeviceGeneric::InitStatus AudioDeviceIOS::Init() {
  ...
  SLRRTCAudioSessionConfiguration* config = [SLRRTCAudioSessionConfiguration webRTCConfiguration];
...
int32_t AudioDeviceIOS::StartPlayout() {
  ...
  if (!recording_ && audio_unit_->GetState() == VoiceProcessingAudioUnit::kInitialized) {
    if (!audio_unit_->Start()) {
      SLRRTCLogError(@"StartPlayout failed to start audio unit.");
      return -1;
    }
...
int32_t AudioDeviceIOS::StopPlayout() {
  ...
  RTC_HISTOGRAM_COUNTS_100000("WebRTC.Audio.AveragePlayoutCallbacksBetweenGlitches",
                              average_number_of_playout_callbacks_between_glitches);
  SLRRTCLog(@"Average number of playout callbacks between glitches: %d",
         average_number_of_playout_callbacks_between_glitches);
  return 0;
}
...
int32_t AudioDeviceIOS::StartRecording() {
  ...
  if (!playing_ && audio_unit_->GetState() == VoiceProcessingAudioUnit::kInitialized) {
    if (!audio_unit_->Start()) {
      SLRRTCLogError(@"StartRecording failed to start audio unit.");
      return -1;
    }
    RTC_LOG(LS_INFO) << "Voice-Processing I/O audio unit is now started";
  }
...
OSStatus AudioDeviceIOS::OnDeliverRecordedData(AudioUnitRenderActionFlags* flags,
  ...
  if (result != noErr) {
    SLRRTCLogError(@"Failed to render audio.");
    return result;
  }
...
OSStatus AudioDeviceIOS::OnGetPlayoutData(AudioUnitRenderActionFlags* flags,
  ...
  if (time_stamp->mSampleTime != num_frames) {
    ...
    if (delta_time > glitch_threshold) {
      SLRRTCLogWarning(@"Possible playout audio glitch detected.\n"
      ...
      if (glitch_threshold < 120 && delta_time > 120) {
        SLRRTCLog(@"Glitch warning is ignored. Probably caused by device switch.");
      } else {
...
void AudioDeviceIOS::HandleInterruptionBegin() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  SLRRTCLog(@"Interruption begin. IsInterrupted changed from %d to 1.", is_interrupted_);
  if (audio_unit_ && audio_unit_->GetState() == VoiceProcessingAudioUnit::kStarted) {
    SLRRTCLog(@"Stopping the audio unit due to interruption begin.");
    if (!audio_unit_->Stop()) {
      SLRRTCLogError(@"Failed to stop the audio unit for interruption begin.");
    } else {
      PrepareForNewStart();
    }
  }
...
void AudioDeviceIOS::HandleInterruptionEnd() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  SLRRTCLog(@"Interruption ended. IsInterrupted changed from %d to 0. "
          "Updating audio unit state.",
         is_interrupted_);
  is_interrupted_ = false;
  UpdateAudioUnit([SLRRTCAudioSession sharedInstance].canPlayOrRecord);
}
...
void AudioDeviceIOS::HandleCanPlayOrRecordChange(bool can_play_or_record) {
  SLRRTCLog(@"Handling CanPlayOrRecord change to: %d", can_play_or_record);
  UpdateAudioUnit(can_play_or_record);
}

void AudioDeviceIOS::HandleSampleRateChange(float sample_rate) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  SLRRTCLog(@"Handling sample rate change to %f.", sample_rate);

  // Don't do anything if we're interrupted.
  if (is_interrupted_) {
    SLRRTCLog(@"Ignoring sample rate change to %f due to interruption.", sample_rate);
    return;
  }
  ...
  SLRRTCLog(@"Handling playout sample rate change to: %f\n"
  ...
  if (std::abs(current_sample_rate - session_sample_rate) <= DBL_EPSILON &&
    ...
    SLRRTCLog(@"Ignoring sample rate change since audio parameters are intact.");
    return;
  }

  // Extra sanity check to ensure that the new sample rate is valid.
  if (session_sample_rate <= 0.0) {
    SLRRTCLogError(@"Sample rate is invalid: %f", session_sample_rate);
    return;
  }
  ...
  SLRRTCLog(@"Stopping and uninitializing audio unit to adjust buffers.");
  ...
  if (!audio_unit_->Initialize(session_sample_rate)) {
    SLRRTCLogError(@"Failed to initialize the audio unit with sample rate: %f", session_sample_rate);
    return;
  }

  // Restart the audio unit if it was already running.
  if (restart_audio_unit && !audio_unit_->Start()) {
    SLRRTCLogError(@"Failed to start audio unit with sample rate: %f", session_sample_rate);
    return;
  }
  SLRRTCLog(@"Successfully handled sample rate change.");
}
...
void AudioDeviceIOS::HandlePlayoutGlitchDetected() {
  ...
  // in this state.
  if (is_interrupted_) {
    SLRRTCLog(@"Ignoring audio glitch due to interruption.");
    return;
  }
  ...
  if (last_output_volume_change_time_ > 0 &&
    ...
    SLRRTCLog(@"Ignoring audio glitch due to recent output volume change.");
    return;
  }
  num_detected_playout_glitches_++;
  SLRRTCLog(@"Number of detected playout glitches: %lld", num_detected_playout_glitches_);
...
void AudioDeviceIOS::HandleOutputVolumeChange() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  SLRRTCLog(@"Output volume change detected.");
...
void AudioDeviceIOS::SetupAudioBuffersForActiveAudioSession() {
  ...
  // 16kHz.
  SLRRTCAudioSessionConfiguration* webRTCConfig = [SLRRTCAudioSessionConfiguration webRTCConfiguration];
  ...
  if (sample_rate <= DBL_EPSILON && playout_parameters_.sample_rate() > 0) {
    SLRRTCLogError(@"Reported rate is invalid: %f. "
...
void AudioDeviceIOS::UpdateAudioUnit(bool can_play_or_record) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  SLRRTCLog(@"Updating audio unit state. CanPlayOrRecord=%d IsInterrupted=%d",
         can_play_or_record,
         is_interrupted_);

  if (is_interrupted_) {
    SLRRTCLog(@"Ignoring audio unit update due to interruption.");
    return;
  }
  ...
  switch (audio_unit_->GetState()) {
    case VoiceProcessingAudioUnit::kInitRequired:
      SLRRTCLog(@"VPAU state: InitRequired");
      RTC_NOTREACHED();
      break;
    case VoiceProcessingAudioUnit::kUninitialized:
      SLRRTCLog(@"VPAU state: Uninitialized");
    ...
    case VoiceProcessingAudioUnit::kInitialized:
      SLRRTCLog(@"VPAU state: Initialized");
  ...
  if (should_initialize_audio_unit) {
    SLRRTCLog(@"Initializing audio unit for UpdateAudioUnit");
    ConfigureAudioSession();
    SetupAudioBuffersForActiveAudioSession();
    if (!audio_unit_->Initialize(playout_parameters_.sample_rate())) {
      SLRRTCLogError(@"Failed to initialize audio unit.");
      return;
    }
  }

  if (should_start_audio_unit) {
    SLRRTCLog(@"Starting audio unit for UpdateAudioUnit");
    // Log session settings before trying to start audio streaming.
    SLRRTCAudioSession* session = [SLRRTCAudioSession sharedInstance];
    SLRRTCLog(@"%@", session);
    if (!audio_unit_->Start()) {
      SLRRTCLogError(@"Failed to start audio unit.");
      return;
    }
  }

  if (should_stop_audio_unit) {
    SLRRTCLog(@"Stopping audio unit for UpdateAudioUnit");
    if (!audio_unit_->Stop()) {
      SLRRTCLogError(@"Failed to stop audio unit.");
      return;
    }
  }

  if (should_uninitialize_audio_unit) {
    SLRRTCLog(@"Uninitializing audio unit for UpdateAudioUnit");
    audio_unit_->Uninitialize();
...
bool AudioDeviceIOS::ConfigureAudioSession() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  SLRRTCLog(@"Configuring audio session.");
  if (has_configured_session_) {
    SLRRTCLogWarning(@"Audio session already configured.");
    return false;
  }
  SLRRTCAudioSession* session = [SLRRTCAudioSession sharedInstance];
  ...
  if (success) {
    ...
    SLRRTCLog(@"Configured audio session.");
  } else {
    SLRRTCLog(@"Failed to configure audio session.");
  }
  return success;
}
...
void AudioDeviceIOS::UnconfigureAudioSession() {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  SLRRTCLog(@"Unconfiguring audio session.");
  if (!has_configured_session_) {
    SLRRTCLogWarning(@"Audio session already unconfigured.");
    return;
  }
  SLRRTCAudioSession* session = [SLRRTCAudioSession sharedInstance];
  [session lockForConfiguration];
  ...
  [session unlockForConfiguration];
  has_configured_session_ = false;
  SLRRTCLog(@"Unconfigured audio session.");
}
...
bool AudioDeviceIOS::InitPlayOrRecord() {
  ...
  if (![session beginWebRTCSession:&error]) {
    [session unlockForConfiguration];
    SLRRTCLogError(@"Failed to begin WebRTC session: %@", error.localizedDescription);
    audio_unit_.reset();
    return false;
  }
