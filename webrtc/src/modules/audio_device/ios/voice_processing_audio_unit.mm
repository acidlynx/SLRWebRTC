...
static void LogStreamDescription(AudioStreamBasicDescription description) {
  ...
  formatIdString[4] = '\0';
  SLRRTCLog(@"AudioStreamBasicDescription: {\n"
          "  mSampleRate: %.2f\n"
          "  formatIDString: %s\n"
          "  mFormatFlags: 0x%X\n"
          "  mBytesPerPacket: %u\n"
...
static OSStatus GetAGCState(AudioUnit audio_unit, UInt32* enabled) {
  ...
  OSStatus result = AudioUnitGetProperty(audio_unit,
                                         ...
                                         enabled,
                                         &size);
  SLRRTCLog(@"VPIO unit AGC: %u", static_cast<unsigned int>(*enabled));
  return result;
}
...
bool VoiceProcessingAudioUnit::Init() {
  ...
  if (result != noErr) {
    vpio_unit_ = nullptr;
    SLRRTCLogError(@"AudioComponentInstanceNew failed. Error=%ld.", (long)result);
    return false;
  }
  ...
  if (result != noErr) {
    DisposeAudioUnit();
    SLRRTCLogError(@"Failed to enable input on input scope of input element. "
                 "Error=%ld.",
                (long)result);
    return false;
  }
  ...
  if (result != noErr) {
    DisposeAudioUnit();
    SLRRTCLogError(@"Failed to enable output on output scope of output element. "
                 "Error=%ld.",
                (long)result);
    return false;
  }
  ...
  if (result != noErr) {
    DisposeAudioUnit();
    SLRRTCLogError(@"Failed to specify the render callback on the output bus. "
                 "Error=%ld.",
                (long)result);
    return false;
  }
  ...
  if (result != noErr) {
    DisposeAudioUnit();
    SLRRTCLogError(@"Failed to disable buffer allocation on the input bus. "
                 "Error=%ld.",
                (long)result);
    return false;
  }
  ...
  if (result != noErr) {
    DisposeAudioUnit();
    SLRRTCLogError(@"Failed to specify the input callback on the input bus. "
                 "Error=%ld.",
                (long)result);
    return false;
  }
...
bool VoiceProcessingAudioUnit::Initialize(Float64 sample_rate) {
  ...
  SLRRTCLog(@"Initializing audio unit with sample rate: %f", sample_rate);
  ...
  if (result != noErr) {
    SLRRTCLogError(@"Failed to set format on output scope of input bus. "
                 "Error=%ld.",
                (long)result);
    return false;
  }
  ...
  if (result != noErr) {
    SLRRTCLogError(@"Failed to set format on input scope of output bus. "
                 "Error=%ld.",
                (long)result);
    return false;
  }
  ...
  while (result != noErr) {
    SLRRTCLogError(@"Failed to initialize the Voice Processing I/O unit. "
                 "Error=%ld.",
                (long)result);
    ++failed_initalize_attempts;
    if (failed_initalize_attempts == kMaxNumberOfAudioUnitInitializeAttempts) {
      // Max number of initialization attempts exceeded, hence abort.
      SLRRTCLogError(@"Too many initialization attempts.");
      return false;
    }
    SLRRTCLog(@"Pause 100ms and try audio unit initialization again...");
    [NSThread sleepForTimeInterval:0.1f];
    result = AudioUnitInitialize(vpio_unit_);
  }
  if (result == noErr) {
    SLRRTCLog(@"Voice Processing I/O unit is now initialized.");
  }
  ...
  if (result != noErr) {
    SLRRTCLogError(@"Failed to get AGC state (1st attempt). "
                 "Error=%ld.",
                (long)result);
    ...
    result =
        AudioUnitSetProperty(vpio_unit_,
                             ...
                             sizeof(enable_agc));
    if (result != noErr) {
      SLRRTCLogError(@"Failed to enable the built-in AGC. "
                   "Error=%ld.",
                  (long)result);
      RTC_HISTOGRAM_COUNTS_SPARSE_100000(
    ...
    result = GetAGCState(vpio_unit_, &agc_is_enabled);
    if (result != noErr) {
      SLRRTCLogError(@"Failed to get AGC state (2nd attempt). "
                   "Error=%ld.",
                  (long)result);
      RTC_HISTOGRAM_COUNTS_SPARSE_100000(
  ...
  SLRRTCLog(@"WebRTC.Audio.BuiltInAGCWasEnabledByDefault: %d",
         agc_was_enabled_by_default);
  ...
  SLRRTCLog(@"WebRTC.Audio.BuiltInAGCIsEnabled: %u",
...
bool VoiceProcessingAudioUnit::Start() {
  RTC_DCHECK_GE(state_, kUninitialized);
  SLRRTCLog(@"Starting audio unit.");

  OSStatus result = AudioOutputUnitStart(vpio_unit_);
  if (result != noErr) {
    SLRRTCLogError(@"Failed to start audio unit. Error=%ld", (long)result);
    return false;
  } else {
    SLRRTCLog(@"Started audio unit");
  }
  state_ = kStarted;
  return true;
}
...
bool VoiceProcessingAudioUnit::Stop() {
  RTC_DCHECK_GE(state_, kUninitialized);
  SLRRTCLog(@"Stopping audio unit.");

  OSStatus result = AudioOutputUnitStop(vpio_unit_);
  if (result != noErr) {
    SLRRTCLogError(@"Failed to stop audio unit. Error=%ld", (long)result);
    return false;
  } else {
    SLRRTCLog(@"Stopped audio unit");
  }
...
bool VoiceProcessingAudioUnit::Uninitialize() {
  RTC_DCHECK_GE(state_, kUninitialized);
  SLRRTCLog(@"Unintializing audio unit.");

  OSStatus result = AudioUnitUninitialize(vpio_unit_);
  if (result != noErr) {
    SLRRTCLogError(@"Failed to uninitialize audio unit. Error=%ld", (long)result);
    return false;
  } else {
    SLRRTCLog(@"Uninitialized audio unit.");
  }
...
OSStatus VoiceProcessingAudioUnit::Render(AudioUnitRenderActionFlags* flags,
  ...
  if (result != noErr) {
    SLRRTCLogError(@"Failed to render audio unit. Error=%ld", (long)result);
  }
  return result;
}
...
void VoiceProcessingAudioUnit::DisposeAudioUnit() {
  if (vpio_unit_) {
    ...
    SLRRTCLog(@"Disposing audio unit.");
    OSStatus result = AudioComponentInstanceDispose(vpio_unit_);
    if (result != noErr) {
      SLRRTCLogError(@"AudioComponentInstanceDispose failed. Error=%ld.",
                  (long)result);
    }
    vpio_unit_ = nullptr;
  }
