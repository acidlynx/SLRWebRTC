...
// Expecting that SLRRTCP feedback is sent uniformly within [0.5, 1.5]s intervals.
...
void SendSideBandwidthEstimation::UpdateRtt(TimeDelta rtt, Timestamp at_time) {
  // Update RTT if we were able to compute an RTT based on this SLRRTCP.
  // FlexFEC doesn't send SLRRTCP SR, which means we won't be able to compute RTT.
