...
void ModuleRtpRtcpImpl::Process() {
  ...
  if (rtcp_sender_.Sending()) {
    ...
    if (rtcp_receiver_.LastReceivedReportBlockMs() > last_rtt_process_time_ &&
        process_rtt) {
      std::vector<SLRRTCPReportBlock> receive_blocks;
      rtcp_receiver_.StatisticsReceived(&receive_blocks);
      int64_t max_rtt = 0;
      for (std::vector<SLRRTCPReportBlock>::iterator it = receive_blocks.begin();
...
int32_t ModuleRtpRtcpImpl::RemoteRTCPStat(
    std::vector<SLRRTCPReportBlock>* receive_blocks) const {
...
void ModuleRtpRtcpImpl::OnReceivedRtcpReportBlocks(
  ...
  if (ack_observer_) {
    ...
    for (const SLRRTCPReportBlock& report_block : report_blocks) {
