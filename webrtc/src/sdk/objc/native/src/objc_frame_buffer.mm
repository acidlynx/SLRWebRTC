...
#import "sdk/objc/api/video_frame_buffer/SLRRTCNativeI420Buffer+Private.h"

namespace webrtc {

namespace {

/** ObjCFrameBuffer that conforms to I420BufferInterface by wrapping SLRRTCI420Buffer */
class ObjCI420FrameBuffer : public I420BufferInterface {
 public:
  explicit ObjCI420FrameBuffer(id<SLRRTCI420Buffer> frame_buffer)
 ...
 private:
  id<SLRRTCI420Buffer> frame_buffer_;
  int width_;
  int height_;
};
...
ObjCFrameBuffer::ObjCFrameBuffer(id<SLRRTCVideoFrameBuffer> frame_buffer)
    : frame_buffer_(frame_buffer), width_(frame_buffer.width), height_(frame_buffer.height) {}
...
id<SLRRTCVideoFrameBuffer> ObjCFrameBuffer::wrapped_frame_buffer() const {
  return frame_buffer_;
}

id<SLRRTCVideoFrameBuffer> ToObjCVideoFrameBuffer(const rtc::scoped_refptr<VideoFrameBuffer>& buffer) {
  if (buffer->type() == VideoFrameBuffer::Type::kNative) {
    return static_cast<ObjCFrameBuffer*>(buffer.get())->wrapped_frame_buffer();
  } else {
    return [[SLRRTCI420Buffer alloc] initWithFrameBuffer:buffer->ToI420()];
  }
}
