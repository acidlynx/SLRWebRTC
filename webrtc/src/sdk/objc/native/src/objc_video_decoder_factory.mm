...
#import "base/SLRRTCVideoDecoder.h"
#import "base/SLRRTCVideoDecoderFactory.h"
#import "base/SLRRTCVideoFrame.h"
#import "base/SLRRTCVideoFrameBuffer.h"
#import "components/video_codec/SLRRTCCodecSpecificInfoH264.h"
...
#import "sdk/objc/api/video_codec/SLRRTCWrappedNativeVideoDecoder.h"
...
class ObjCVideoDecoder : public VideoDecoder {
 public:
  ObjCVideoDecoder(id<SLRRTCVideoDecoder> decoder)
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
      SLRRTCVideoEncoderSettings *settings = [[SLRRTCVideoEncoderSettings alloc] init];
...
#pragma clang diagnostic pop
  ...
  int32_t Decode(const EncodedImage &input_image,
    ...
    SLRRTCEncodedImage *encodedImage =
        [[SLRRTCEncodedImage alloc] initWithNativeEncodedImage:input_image];

    // webrtc::CodecSpecificInfo only handles a hard coded list of codecs
    id<SLRRTCCodecSpecificInfo> rtcCodecSpecificInfo = nil;
    if (codec_specific_info) {
      if (codec_specific_info->codecType == kVideoCodecH264) {
        SLRRTCCodecSpecificInfoH264 *h264Info = [[SLRRTCCodecSpecificInfoH264 alloc] init];
        h264Info.packetizationMode =
            (SLRRTCH264PacketizationMode)codec_specific_info->codecSpecific.H264.packetization_mode;
  ...
  int32_t RegisterDecodeCompleteCallback(DecodedImageCallback *callback) override {
    [decoder_ setCallback:^(SLRRTCVideoFrame *frame) {
 ...
 private:
  id<SLRRTCVideoDecoder> decoder_;
  const std::string implementation_name_;
};
}  // namespace

ObjCVideoDecoderFactory::ObjCVideoDecoderFactory(id<SLRRTCVideoDecoderFactory> decoder_factory)
    : decoder_factory_(decoder_factory) {}

ObjCVideoDecoderFactory::~ObjCVideoDecoderFactory() {}

id<SLRRTCVideoDecoderFactory> ObjCVideoDecoderFactory::wrapped_decoder_factory() const {
...
std::unique_ptr<VideoDecoder> ObjCVideoDecoderFactory::CreateVideoDecoder(
  ...
  for (SLRRTCVideoCodecInfo *codecInfo in decoder_factory_.supportedCodecs) {
    if ([codecName isEqualToString:codecInfo.name]) {
      id<SLRRTCVideoDecoder> decoder = [decoder_factory_ createDecoder:codecInfo];

      if ([decoder isKindOfClass:[SLRRTCWrappedNativeVideoDecoder class]]) {
        return [(SLRRTCWrappedNativeVideoDecoder *)decoder releaseWrappedDecoder];
      } else {
...
std::vector<SdpVideoFormat> ObjCVideoDecoderFactory::GetSupportedFormats() const {
  ...
  for (SLRRTCVideoCodecInfo *supportedCodec in decoder_factory_.supportedCodecs) {
