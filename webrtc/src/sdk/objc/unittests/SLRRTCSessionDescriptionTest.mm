...
#import "api/peerconnection/SLRRTCSessionDescription+Private.h"
#import "api/peerconnection/SLRRTCSessionDescription.h"
#import "helpers/NSString+StdString.h"

@interface SLRRTCSessionDescriptionTest : NSObject
- (void)testSessionDescriptionConversion;
- (void)testInitFromNativeSessionDescription;
@end

@implementation SLRRTCSessionDescriptionTest

/**
 * Test conversion of an Objective-C SLRRTCSessionDescription to a native
 * SessionDescriptionInterface (based on the types and SDP strings being equal).
 */
- (void)testSessionDescriptionConversion {
  SLRRTCSessionDescription *description =
      [[SLRRTCSessionDescription alloc] initWithType:SLRRTCSdpTypeAnswer
                                              sdp:[self sdp]];

  webrtc::SessionDescriptionInterface *nativeDescription =
      description.nativeDescription;

  EXPECT_EQ(SLRRTCSdpTypeAnswer,
      [SLRRTCSessionDescription typeForStdString:nativeDescription->type()]);
...
- (void)testInitFromNativeSessionDescription {
  ...
  EXPECT_EQ(webrtc::SessionDescriptionInterface::kAnswer,
      [SLRRTCSessionDescription stdStringForType:description.type]);
...
TEST(SLRRTCSessionDescriptionTest, SessionDescriptionConversionTest) {
  @autoreleasepool {
    SLRRTCSessionDescriptionTest *test = [[SLRRTCSessionDescriptionTest alloc] init];
    [test testSessionDescriptionConversion];
  }
}

TEST(SLRRTCSessionDescriptionTest, InitFromSessionDescriptionTest) {
  @autoreleasepool {
    SLRRTCSessionDescriptionTest *test = [[SLRRTCSessionDescriptionTest alloc] init];
