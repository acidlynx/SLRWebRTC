...
id<SLRRTCVideoDecoderFactory> CreateDecoderFactoryReturning(int return_code) {
  id decoderMock = OCMProtocolMock(@protocol(SLRRTCVideoDecoder));
  ...
  id decoderFactoryMock = OCMProtocolMock(@protocol(SLRRTCVideoDecoderFactory));
  SLRRTCVideoCodecInfo *supported = [[SLRRTCVideoCodecInfo alloc] initWithName:@"H264" parameters:nil];
...
id<SLRRTCVideoDecoderFactory> CreateOKDecoderFactory() {
  return CreateDecoderFactoryReturning(WEBRTC_VIDEO_CODEC_OK);
}

id<SLRRTCVideoDecoderFactory> CreateErrorDecoderFactory() {
  return CreateDecoderFactoryReturning(WEBRTC_VIDEO_CODEC_ERROR);
}

std::unique_ptr<webrtc::VideoDecoder> GetObjCDecoder(id<SLRRTCVideoDecoderFactory> factory) {
