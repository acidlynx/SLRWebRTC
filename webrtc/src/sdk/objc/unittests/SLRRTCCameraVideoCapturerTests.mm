...
#import "components/capturer/SLRRTCCameraVideoCapturer.h"
#import "helpers/AVCaptureSession+DevicePosition.h"
#import "helpers/SLRRTCDispatcher.h"
...
#endif
@interface SLRRTCCameraVideoCapturer (Tests)<AVCaptureVideoDataOutputSampleBufferDelegate>
- (instancetype)initWithDelegate:(__weak id<SLRRTCVideoCapturerDelegate>)delegate
                  captureSession:(AVCaptureSession *)captureSession;
@end

@interface SLRRTCCameraVideoCapturerTests : NSObject
...
@property(nonatomic, strong) SLRRTCCameraVideoCapturer *capturer;
@end

@implementation SLRRTCCameraVideoCapturerTests
...
- (void)setup {
  self.delegateMock = OCMProtocolMock(@protocol(SLRRTCVideoCapturerDelegate));
  self.captureConnectionMock = OCMClassMock([AVCaptureConnection class]);
  self.capturer = [[SLRRTCCameraVideoCapturer alloc] initWithDelegate:self.delegateMock];
...
- (void)testSupportedFormatsForDevice {
  ...
  // when
  NSArray *supportedFormats = [SLRRTCCameraVideoCapturer supportedFormatsForDevice:self.deviceMock];
...
- (void)testDelegateCallbackWithValidBufferAndOrientationUpdate {
  ...
  [[self.delegateMock expect] capturer:self.capturer
                  didCaptureVideoFrame:[OCMArg checkWithBlock:^BOOL(SLRRTCVideoFrame *expectedFrame) {
                    EXPECT_EQ(expectedFrame.rotation, SLRRTCVideoRotation_270);
                    return YES;
                  }]];
  ...
  [[self.delegateMock expect] capturer:self.capturer
    ...
    if (camera == AVCaptureDevicePositionFront) {
      if (deviceOrientation == UIDeviceOrientationLandscapeLeft) {
        EXPECT_EQ(expectedFrame.rotation, SLRRTCVideoRotation_180);
      } else if (deviceOrientation == UIDeviceOrientationLandscapeRight) {
        EXPECT_EQ(expectedFrame.rotation, SLRRTCVideoRotation_0);
      }
...
TEST(SLRRTCCameraVideoCapturerTests, SetupSession) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
  [test testSetupSession];
  [test tearDown];
}

TEST(SLRRTCCameraVideoCapturerTests, SetupSessionOutput) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
...
TEST(SLRRTCCameraVideoCapturerTests, SupportedFormatsForDevice) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
...
TEST(SLRRTCCameraVideoCapturerTests, DelegateCallbackNotCalledWhenInvalidBuffer) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
...
TEST(SLRRTCCameraVideoCapturerTests, DelegateCallbackWithValidBufferAndOrientationUpdate) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
...
TEST(SLRRTCCameraVideoCapturerTests, RotationCameraBackLandscapeLeft) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
...
TEST(SLRRTCCameraVideoCapturerTests, RotationCameraFrontLandscapeLeft) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
...
TEST(SLRRTCCameraVideoCapturerTests, RotationCameraBackLandscapeRight) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
  [test testRotationCamera:AVCaptureDevicePositionBack
...
TEST(SLRRTCCameraVideoCapturerTests, RotationCameraFrontLandscapeRight) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
  [test testRotationCamera:AVCaptureDevicePositionFront
           withOrientation:UIDeviceOrientationLandscapeRight];
...
TEST(SLRRTCCameraVideoCapturerTests, RotationCameraFrame) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
...
TEST(SLRRTCCameraVideoCapturerTests, ImageExif) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setup];
  [test testImageExif];
  [test tearDown];
...
TEST(SLRRTCCameraVideoCapturerTests, StartAndStopCapture) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
...
TEST(SLRRTCCameraVideoCapturerTests, StartCaptureFailingToLockForConfiguration) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setupWithMockedCaptureSession];
...
TEST(SLRRTCCameraVideoCapturerTests, StartAndStopCaptureWithCallbacks) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setupWithMockedCaptureSession];
...
TEST(SLRRTCCameraVideoCapturerTests, StartCaptureFailingToLockForConfigurationWithCallback) {
  SLRRTCCameraVideoCapturerTests *test = [[SLRRTCCameraVideoCapturerTests alloc] init];
  [test setupWithMockedCaptureSession];
