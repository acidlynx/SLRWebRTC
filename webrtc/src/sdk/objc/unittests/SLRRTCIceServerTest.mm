...
#import "api/peerconnection/SLRRTCIceServer+Private.h"
#import "api/peerconnection/SLRRTCIceServer.h"
#import "helpers/NSString+StdString.h"

@interface SLRRTCIceServerTest : NSObject
...
@implementation SLRRTCIceServerTest

- (void)testOneURLServer {
  SLRRTCIceServer *server = [[SLRRTCIceServer alloc] initWithURLStrings:@[
...
- (void)testPasswordCredential {
  SLRRTCIceServer *server = [[SLRRTCIceServer alloc]
...
- (void)testHostname {
  SLRRTCIceServer *server = [[SLRRTCIceServer alloc] initWithURLStrings:@[ @"turn1:turn1.example.net" ]
                                                         username:@"username"
                                                       credential:@"credential"
                                                    tlsCertPolicy:SLRRTCTlsCertPolicySecure
                                                         hostname:@"hostname"];
  webrtc::PeerConnectionInterface::IceServer iceStruct = server.nativeServer;
  EXPECT_EQ(1u, iceStruct.urls.size());
  EXPECT_EQ("turn1:turn1.example.net", iceStruct.urls.front());
...
- (void)testInitFromNativeServer {
  ...
  SLRRTCIceServer *iceServer =
      [[SLRRTCIceServer alloc] initWithNativeServer:nativeServer];
...
TEST(SLRRTCIceServerTest, OneURLTest) {
  @autoreleasepool {
    SLRRTCIceServerTest *test = [[SLRRTCIceServerTest alloc] init];
    [test testOneURLServer];
  }
}

TEST(SLRRTCIceServerTest, TwoURLTest) {
  @autoreleasepool {
    SLRRTCIceServerTest *test = [[SLRRTCIceServerTest alloc] init];
...
TEST(SLRRTCIceServerTest, PasswordCredentialTest) {
  @autoreleasepool {
    SLRRTCIceServerTest *test = [[SLRRTCIceServerTest alloc] init];
...
TEST(SLRRTCIceServerTest, HostnameTest) {
  @autoreleasepool {
    SLRRTCIceServerTest *test = [[SLRRTCIceServerTest alloc] init];
    [test testHostname];
  }
...
TEST(SLRRTCIceServerTest, InitFromNativeServerTest) {
  @autoreleasepool {
    SLRRTCIceServerTest *test = [[SLRRTCIceServerTest alloc] init];
