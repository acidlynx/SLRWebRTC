...
#import "api/peerconnection/SLRRTCMediaConstraints+Private.h"
#import "api/peerconnection/SLRRTCMediaConstraints.h"
#import "helpers/NSString+StdString.h"

@interface SLRRTCMediaConstraintsTest : NSObject
- (void)testMediaConstraints;
@end

@implementation SLRRTCMediaConstraintsTest

- (void)testMediaConstraints {
  NSDictionary *mandatory = @{@"key1": @"value1", @"key2": @"value2"};
  NSDictionary *optional = @{@"key3": @"value3", @"key4": @"value4"};

  SLRRTCMediaConstraints *constraints = [[SLRRTCMediaConstraints alloc]
...
TEST(SLRRTCMediaConstraintsTest, MediaConstraintsTest) {
  @autoreleasepool {
    SLRRTCMediaConstraintsTest *test = [[SLRRTCMediaConstraintsTest alloc] init];
