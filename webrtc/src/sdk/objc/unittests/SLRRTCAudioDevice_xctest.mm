...
@interface SLRRTCAudioDeviceTests: XCTestCase {
  rtc::scoped_refptr<webrtc::AudioDeviceModule> _audioDeviceModule;
  std::unique_ptr<webrtc::ios_adm::AudioDeviceIOS> _audio_device;
}

@property(nonatomic) SLRRTCAudioSession *audioSession;

@end

@implementation SLRRTCAudioDeviceTests
...
- (void)setUp {
  ...
  self.audioSession = [SLRRTCAudioSession sharedInstance];
...
// AudioDeviceIOS listens to SLRRTCAudioSession interrupted notifications by:
// - In AudioDeviceIOS.InitPlayOrRecord registers its audio_session_observer_
//   callback with SLRRTCAudioSession's delegate list.
// - When SLRRTCAudioSession receives an iOS audio interrupted notification, it
...
//   audio_session_observer_ callback is removed from SLRRTCAudioSessions's
//   delegate list.
//   So if SLRRTCAudioSession receives an iOS end audio interruption notification,
//   AudioDeviceIOS is not notified as its callback is not in SLRRTCAudioSession's
...
// As SLRRTCAudioSession keeps its own interrupted state, the fix is to initialize
// AudioDeviceIOS's is_interrupted_ flag to SLRRTCAudioSession's isInterrupted
