...
#import "components/capturer/SLRRTCFileVideoCapturer.h"
...
@interface MockCapturerDelegate : NSObject <SLRRTCVideoCapturerDelegate>
...
@interface SLRRTCFileVideoCapturerTests : XCTestCase

@property(nonatomic, strong) SLRRTCFileVideoCapturer *capturer;
@property(nonatomic, strong) MockCapturerDelegate *mockDelegate;

@end

@implementation SLRRTCFileVideoCapturerTests
@synthesize capturer = _capturer;
@synthesize mockDelegate = _mockDelegate;

- (void)setUp {
  self.mockDelegate = [[MockCapturerDelegate alloc] init];
  self.capturer = [[SLRRTCFileVideoCapturer alloc] initWithDelegate:self.mockDelegate];
}
...
- (void)testCaptureWhenFileNotInBundle {
  ...
  SLRRTCFileVideoCapturerErrorBlock errorBlock = ^void(NSError *error) {
    errorOccured = YES;
  };
...
- (void)testSecondStartCaptureCallFails {
  ...
  SLRRTCFileVideoCapturerErrorBlock firstErrorBlock = ^void(NSError *error) {
    // This block should never be called.
    NSLog(@"Error: %@", [error userInfo]);
    ASSERT_TRUE(false);
  };

  SLRRTCFileVideoCapturerErrorBlock secondErrorBlock = ^void(NSError *error) {
    secondError = YES;
  };
