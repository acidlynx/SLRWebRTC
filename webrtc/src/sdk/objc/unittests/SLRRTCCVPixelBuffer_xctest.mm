...
@interface SLRRTCCVPixelBufferTests : XCTestCase
@end

@implementation SLRRTCCVPixelBufferTests {
}
...
- (void)testRequiresCroppingWithCrop {
  ...
  CVPixelBufferCreate(
      NULL, 720, 1280, kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange, NULL, &pixelBufferRef);
  SLRRTCCVPixelBuffer *croppedBuffer = [[SLRRTCCVPixelBuffer alloc] initWithPixelBuffer:pixelBufferRef
                                                                     adaptedWidth:720
                                                                    adaptedHeight:1280
                                                                        cropWidth:360
                                                                       cropHeight:640
...
- (void)cropAndScaleTestWithNV12InputFormat:(OSType)inputFormat
  ...
  SLRRTCCVPixelBuffer *scaledBuffer =
      [[SLRRTCCVPixelBuffer alloc] initWithPixelBuffer:outputPixelBufferRef];
  XCTAssertEqual(scaledBuffer.width, outputSize.width);
  XCTAssertEqual(scaledBuffer.height, outputSize.height);

  if (outputSize.width > 0 && outputSize.height > 0) {
    SLRRTCI420Buffer *originalBufferI420 = [buffer toI420];
    SLRRTCI420Buffer *scaledBufferI420 = [scaledBuffer toI420];
    double psnr =
...
- (void)cropAndScaleTestWithRGBPixelFormat:(OSType)pixelFormat cropX:(int)cropX cropY:(int)cropY {
  ...
  SLRRTCCVPixelBuffer *buffer =
      [[SLRRTCCVPixelBuffer alloc] initWithPixelBuffer:pixelBufferRef
...
- (void)toI420WithPixelFormat:(OSType)pixelFormat {
  ...
  SLRRTCCVPixelBuffer *buffer = [[SLRRTCCVPixelBuffer alloc] initWithPixelBuffer:pixelBufferRef];
  SLRRTCI420Buffer *fromCVPixelBuffer = [buffer toI420];
