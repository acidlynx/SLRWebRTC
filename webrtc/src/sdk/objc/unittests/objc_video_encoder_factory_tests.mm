...
id<SLRRTCVideoEncoderFactory> CreateEncoderFactoryReturning(int return_code) {
  id encoderMock = OCMProtocolMock(@protocol(SLRRTCVideoEncoder));
  ...
  id encoderFactoryMock = OCMProtocolMock(@protocol(SLRRTCVideoEncoderFactory));
...
id<SLRRTCVideoEncoderFactory> CreateOKEncoderFactory() {
  return CreateEncoderFactoryReturning(WEBRTC_VIDEO_CODEC_OK);
}

id<SLRRTCVideoEncoderFactory> CreateErrorEncoderFactory() {
  return CreateEncoderFactoryReturning(WEBRTC_VIDEO_CODEC_ERROR);
}

std::unique_ptr<webrtc::VideoEncoder> GetObjCEncoder(id<SLRRTCVideoEncoderFactory> factory) {
...
TEST(ObjCVideoEncoderFactoryTest, EncodeReturnsOKOnSuccess) {
  ...
  rtc::scoped_refptr<webrtc::VideoFrameBuffer> buffer =
      new rtc::RefCountedObject<webrtc::ObjCFrameBuffer>(
          [[SLRRTCCVPixelBuffer alloc] initWithPixelBuffer:pixel_buffer]);
