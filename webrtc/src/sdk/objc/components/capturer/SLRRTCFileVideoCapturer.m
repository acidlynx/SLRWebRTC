...
#import "SLRRTCFileVideoCapturer.h"
...
NSString *const kRTCFileVideoCapturerErrorDomain = @"org.webrtc.SLRRTCFileVideoCapturer";

typedef NS_ENUM(NSInteger, SLRRTCFileVideoCapturerErrorCode) {
  SLRRTCFileVideoCapturerErrorCode_CapturerRunning = 2000,
  SLRRTCFileVideoCapturerErrorCode_FileNotFound
};

typedef NS_ENUM(NSInteger, SLRRTCFileVideoCapturerStatus) {
  SLRRTCFileVideoCapturerStatusNotInitialized,
  SLRRTCFileVideoCapturerStatusStarted,
  SLRRTCFileVideoCapturerStatusStopped
};

@interface SLRRTCFileVideoCapturer ()
@property(nonatomic, assign) CMTime lastPresentationTime;
@property(nonatomic, strong) NSURL *fileURL;
@end

@implementation SLRRTCFileVideoCapturer {
  AVAssetReader *_reader;
  AVAssetReaderTrackOutput *_outTrack;
  SLRRTCFileVideoCapturerStatus _status;
...
- (void)startCapturingFromFileNamed:(NSString *)nameOfFile
                            onError:(SLRRTCFileVideoCapturerErrorBlock)errorBlock {
  if (_status == SLRRTCFileVideoCapturerStatusStarted) {
    NSError *error =
        [NSError errorWithDomain:kRTCFileVideoCapturerErrorDomain
                            code:SLRRTCFileVideoCapturerErrorCode_CapturerRunning
                        userInfo:@{NSUnderlyingErrorKey : @"Capturer has been started."}];

    errorBlock(error);
    return;
  } else {
    _status = SLRRTCFileVideoCapturerStatusStarted;
  }
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (!pathForFile) {
      ...
      NSError *error = [NSError errorWithDomain:kRTCFileVideoCapturerErrorDomain
                                           code:SLRRTCFileVideoCapturerErrorCode_FileNotFound
...
- (void)setupReaderOnError:(SLRRTCFileVideoCapturerErrorBlock)errorBlock {
  ...
  SLRRTCLog(@"File capturer started reading");
  [self readNextBuffer];
}
- (void)stopCapture {
  _status = SLRRTCFileVideoCapturerStatusStopped;
  SLRRTCLog(@"File capturer stopped.");
}
...
- (void)readNextBuffer {
  if (_status == SLRRTCFileVideoCapturerStatusStopped) {
...
- (void)publishSampleBuffer:(CMSampleBufferRef)sampleBuffer {
  ...
  dispatch_source_set_event_handler(timer, ^{
    ...
    SLRRTCVideoFrame *videoFrame =
        [[SLRRTCVideoFrame alloc] initWithBuffer:rtcPixelBuffer rotation:0 timeStampNs:timeStampNs];
    CFRelease(sampleBuffer);
