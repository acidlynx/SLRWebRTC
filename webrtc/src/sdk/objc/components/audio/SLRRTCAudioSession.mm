...
#import "SLRRTCAudioSession+Private.h"
...
#import "SLRRTCAudioSessionConfiguration.h"
#import "base/SLRRTCLogging.h"


NSString * const kRTCAudioSessionErrorDomain = @"org.webrtc.SLRRTCAudioSession";
...
@implementation SLRRTCAudioSession {
...
+ (instancetype)sharedInstance {
  ...
  static SLRRTCAudioSession *sharedInstance = nil;
  ...
  if (self = [super init]) {
    ...
    [_session addObserver:self
               forKeyPath:kRTCAudioSessionOutputVolumeSelector
                  ...
                  context:(__bridge void*)SLRRTCAudioSession.class];

    SLRRTCLog(@"SLRRTCAudioSession (%p): init.", self);
  }
  return self;
}
...
- (void)dealloc {
  ...
  [_session removeObserver:self
                forKeyPath:kRTCAudioSessionOutputVolumeSelector
                   context:(__bridge void*)SLRRTCAudioSession.class];
  SLRRTCLog(@"SLRRTCAudioSession (%p): dealloc.", self);
}
...
- (void)addDelegate:(id<SLRRTCAudioSessionDelegate>)delegate {
  SLRRTCLog(@"Adding delegate: (%p)", delegate);
  if (!delegate) {
    return;
  }
  @synchronized(self) {
...
- (void)removeDelegate:(id<SLRRTCAudioSessionDelegate>)delegate {
  SLRRTCLog(@"Removing delegate: (%p)", delegate);
  if (!delegate) {
    return;
  }
  @synchronized(self) {
...
- (BOOL)setActive:(BOOL)active
  ...
  if (!active && activationCount == 0) {
    SLRRTCLogWarning(@"Attempting to deactivate without prior activation.");
  }
  ...
  if (success) {
    ...
    SLRRTCLogError(@"Failed to setActive:%d. Error: %@",
  ...
  SLRRTCLog(@"Number of current activations: %d", _activationCount);
  return success;
}
...
- (void)handleInterruptionNotification:(NSNotification *)notification {
  ...
  switch (type) {
    case AVAudioSessionInterruptionTypeBegan:
      SLRRTCLog(@"Audio session interruption began.");
      self.isActive = NO;
      self.isInterrupted = YES;
      [self notifyDidBeginInterruption];
      break;
    case AVAudioSessionInterruptionTypeEnded: {
      SLRRTCLog(@"Audio session interruption ended.");
...
- (void)handleRouteChangeNotification:(NSNotification *)notification {
  ...
  SLRRTCLog(@"Audio route changed:");
  switch (reason) {
    case AVAudioSessionRouteChangeReasonUnknown:
      SLRRTCLog(@"Audio route changed: ReasonUnknown");
      break;
    case AVAudioSessionRouteChangeReasonNewDeviceAvailable:
      SLRRTCLog(@"Audio route changed: NewDeviceAvailable");
      break;
    case AVAudioSessionRouteChangeReasonOldDeviceUnavailable:
      SLRRTCLog(@"Audio route changed: OldDeviceUnavailable");
      break;
    case AVAudioSessionRouteChangeReasonCategoryChange:
      SLRRTCLog(@"Audio route changed: CategoryChange to :%@",
             self.session.category);
      break;
    case AVAudioSessionRouteChangeReasonOverride:
      SLRRTCLog(@"Audio route changed: Override");
      break;
    case AVAudioSessionRouteChangeReasonWakeFromSleep:
      SLRRTCLog(@"Audio route changed: WakeFromSleep");
      break;
    case AVAudioSessionRouteChangeReasonNoSuitableRouteForCategory:
      SLRRTCLog(@"Audio route changed: NoSuitableRouteForCategory");
      break;
    case AVAudioSessionRouteChangeReasonRouteConfigurationChange:
      SLRRTCLog(@"Audio route changed: RouteConfigurationChange");
      break;
  }
  AVAudioSessionRouteDescription* previousRoute =
      notification.userInfo[AVAudioSessionRouteChangePreviousRouteKey];
  // Log previous route configuration.
  SLRRTCLog(@"Previous route: %@\nCurrent route:%@",
...
- (void)handleMediaServicesWereLost:(NSNotification *)notification {
  SLRRTCLog(@"Media services were lost.");
  [self updateAudioSessionAfterEvent];
  [self notifyMediaServicesWereLost];
}

- (void)handleMediaServicesWereReset:(NSNotification *)notification {
  SLRRTCLog(@"Media services were reset.");
  [self updateAudioSessionAfterEvent];
...
- (void)handleSilenceSecondaryAudioHintNotification:(NSNotification *)notification {
  ...
  switch (type) {
    case AVAudioSessionSilenceSecondaryAudioHintTypeBegin:
      SLRRTCLog(@"Another application's primary audio has started.");
      break;
    case AVAudioSessionSilenceSecondaryAudioHintTypeEnd:
      SLRRTCLog(@"Another application's primary audio has stopped.");
      break;
  }
}
...
- (void)handleApplicationDidBecomeActive:(NSNotification *)notification {
  ...
  SLRRTCLog(@"Application became active after an interruption. Treating as interruption "
...
- (std::vector<__weak id<SLRRTCAudioSessionDelegate> >)delegates {
  @synchronized(self) {
...
// TODO(tkchin): check for duplicates.
- (void)pushDelegate:(id<SLRRTCAudioSessionDelegate>)delegate {
  @synchronized(self) {
...
- (BOOL)checkLock:(NSError **)outError {
  ...
  if (!self.isLocked) {
    if (outError) {
      *outError = [SLRRTCAudioSession lockError];
    }
    return NO;
  }
  return YES;
...
- (BOOL)configureWebRTCSession:(NSError **)outError {
  ...
  SLRRTCLog(@"Configuring audio session for WebRTC.");

  // Configure the AVAudioSession and activate it.
  // Provide an error even if there isn't one so we can log it.
  NSError *error = nil;
  SLRRTCAudioSessionConfiguration *webRTCConfig =
      [SLRRTCAudioSessionConfiguration webRTCConfiguration];
  if (![self setConfiguration:webRTCConfig active:YES error:&error]) {
    SLRRTCLogError(@"Failed to set WebRTC audio configuration: %@",
  ...
  if (!self.inputAvailable) {
    SLRRTCLogError(@"No audio input path is available!");
  ...
  if (sessionSampleRate != preferredSampleRate) {
    ...
    if (![self setPreferredSampleRate:sessionSampleRate
                                error:&error]) {
      SLRRTCLogError(@"Failed to set preferred sample rate: %@",
                  error.localizedDescription);
      if (outError) {
        *outError = error;
      }
...
- (BOOL)unconfigureWebRTCSession:(NSError **)outError {
  ...
  SLRRTCLog(@"Unconfiguring audio session for WebRTC.");
...
- (void)updateAudioSessionAfterEvent {
  ...
  if ([self.session setActive:shouldActivate
    ...
    SLRRTCLogError(@"Failed to set session active to %d. Error:%@",
...
- (void)audioSessionDidActivate:(AVAudioSession *)session {
  if (_session != session) {
    SLRRTCLogError(@"audioSessionDidActivate called on different AVAudioSession");
  }
  SLRRTCLog(@"Audio session was externally activated.");
  [self incrementActivationCount];
  self.isActive = YES;
  ...
  if (self.isInterrupted) {
    SLRRTCLog(@"Clearing interrupted state due to external activation.");
    self.isInterrupted = NO;
  }
...
- (void)audioSessionDidDeactivate:(AVAudioSession *)session {
  if (_session != session) {
    SLRRTCLogError(@"audioSessionDidDeactivate called on different AVAudioSession");
  }
  SLRRTCLog(@"Audio session was externally deactivated.");
  self.isActive = NO;
  [self decrementActivationCount];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if (context == (__bridge void*)SLRRTCAudioSession.class) {
    if (object == _session) {
      NSNumber *newVolume = change[NSKeyValueChangeNewKey];
      SLRRTCLog(@"OutputVolumeDidChange to %f", newVolume.floatValue);
