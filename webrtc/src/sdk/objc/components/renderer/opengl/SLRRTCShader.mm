...
GLuint SLRRTCCreateShader(GLenum type, const GLchar *source) {
...
GLuint SLRRTCCreateProgram(GLuint vertexShader, GLuint fragmentShader) {
...
GLuint SLRRTCCreateProgramFromFragmentSource(const char fragmentShaderSource[]) {
  GLuint vertexShader = SLRRTCCreateShader(GL_VERTEX_SHADER, kRTCVertexShaderSource);
  RTC_CHECK(vertexShader) << "failed to create vertex shader";
  GLuint fragmentShader =
      SLRRTCCreateShader(GL_FRAGMENT_SHADER, fragmentShaderSource);
  RTC_CHECK(fragmentShader) << "failed to create fragment shader";
  GLuint program = SLRRTCCreateProgram(vertexShader, fragmentShader);
...
BOOL SLRRTCCreateVertexBuffer(GLuint *vertexBuffer, GLuint *vertexArray) {
#if !TARGET_OS_IPHONE
...
void SLRRTCSetVertexData(SLRRTCVideoRotation rotation) {
