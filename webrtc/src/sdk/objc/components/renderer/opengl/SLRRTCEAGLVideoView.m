...
#import "SLRRTCEAGLVideoView.h"

#import <GLKit/GLKit.h>

#import "SLRRTCDefaultShader.h"
#import "SLRRTCDisplayLinkTimer.h"
#import "SLRRTCI420TextureCache.h"
#import "SLRRTCNV12TextureCache.h"
...
// SLRRTCEAGLVideoView wraps a GLKView which is setup with
...
// is disabled for the reasons above, the SLRRTCEAGLVideoView maintains
// its own |isDirty| flag.

@interface SLRRTCEAGLVideoView () <GLKViewDelegate>
// |videoFrame| is set when we receive a frame from a worker thread and is read
// from the display link callback so atomicity is required.
@property(atomic, strong) SLRRTCVideoFrame *videoFrame;
@property(nonatomic, readonly) GLKView *glkView;
@end

@implementation SLRRTCEAGLVideoView {
  SLRRTCDisplayLinkTimer *_timer;
  EAGLContext *_glContext;
  // This flag should only be set and read on the main thread (e.g. by
  // setNeedsDisplay)
  BOOL _isDirty;
  id<SLRRTCVideoViewShading> _shader;
  SLRRTCNV12TextureCache *_nv12TextureCache;
  SLRRTCI420TextureCache *_i420TextureCache;
...
- (instancetype)initWithFrame:(CGRect)frame {
  return [self initWithFrame:frame shader:[[SLRRTCDefaultShader alloc] init]];
}

- (instancetype)initWithCoder:(NSCoder *)aDecoder {
  return [self initWithCoder:aDecoder shader:[[SLRRTCDefaultShader alloc] init]];
}

- (instancetype)initWithFrame:(CGRect)frame shader:(id<SLRRTCVideoViewShading>)shader {
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder shader:(id<SLRRTCVideoViewShading>)shader {
...
- (BOOL)configure {
  ...
  if (!glContext) {
    SLRRTCLogError(@"Failed to create EAGLContext");
    return NO;
  }
  ...
  __weak SLRRTCEAGLVideoView *weakSelf = self;
  _timer = [[SLRRTCDisplayLinkTimer alloc] initWithTimerHandler:^{
      SLRRTCEAGLVideoView *strongSelf = weakSelf;
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
  ...
  // one used by |view|.
  SLRRTCVideoFrame *frame = self.videoFrame;
  ...
  if ([frame.buffer isKindOfClass:[SLRRTCCVPixelBuffer class]]) {
    ...
    if (!_i420TextureCache) {
      _i420TextureCache = [[SLRRTCI420TextureCache alloc] initWithContext:_glContext];
    }
...
- (void)renderFrame:(SLRRTCVideoFrame *)frame {
