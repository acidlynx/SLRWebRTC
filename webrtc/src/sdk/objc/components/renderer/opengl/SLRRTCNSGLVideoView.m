...
@interface SLRRTCNSGLVideoView ()
// |videoFrame| is set when we receive a frame from a worker thread and is read
// from the display link callback so atomicity is required.
@property(atomic, strong) SLRRTCVideoFrame *videoFrame;
@property(atomic, strong) SLRRTCI420TextureCache *i420TextureCache;
...
static CVReturn OnDisplayLinkFired(CVDisplayLinkRef displayLink,
  ...
  SLRRTCNSGLVideoView *view = (__bridge SLRRTCNSGLVideoView *)displayLinkContext;
  [view drawFrame];
  return kCVReturnSuccess;
}

@implementation SLRRTCNSGLVideoView {
  CVDisplayLinkRef _displayLink;
  SLRRTCVideoFrame *_lastDrawnFrame;
...
- (instancetype)initWithFrame:(NSRect)frame pixelFormat:(NSOpenGLPixelFormat *)format {
  return [self initWithFrame:frame pixelFormat:format shader:[[SLRRTCDefaultShader alloc] init]];
}

- (instancetype)initWithFrame:(NSRect)frame
                  pixelFormat:(NSOpenGLPixelFormat *)format
                       shader:(id<SLRRTCVideoViewShading>)shader {
...
- (void)drawFrame {
  ...
  if (!self.i420TextureCache) {
    self.i420TextureCache = [[SLRRTCI420TextureCache alloc] initWithContext:context];
  }
  SLRRTCI420TextureCache *i420TextureCache = self.i420TextureCache;
  if (i420TextureCache) {
