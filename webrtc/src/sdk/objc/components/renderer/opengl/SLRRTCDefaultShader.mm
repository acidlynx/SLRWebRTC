...
#import "SLRRTCOpenGLDefines.h"
#import "SLRRTCShader.h"
...
@implementation SLRRTCDefaultShader {
  GLuint _vertexBuffer;
  GLuint _vertexArray;
  // Store current rotation and only upload new vertex data when rotation changes.
  absl::optional<SLRRTCVideoRotation> _currentRotation;
...
- (BOOL)createAndSetupI420Program {
  ...
  _i420Program = SLRRTCCreateProgramFromFragmentSource(kI420FragmentShaderSource);
  if (!_i420Program) {
    return NO;
  }
  ...
  if (ySampler < 0 || uSampler < 0 || vSampler < 0) {
    SLRRTCLog(@"Failed to get uniform variable locations in I420 shader");
    glDeleteProgram(_i420Program);
    _i420Program = 0;
    return NO;
  }
...
- (BOOL)createAndSetupNV12Program {
  ...
  _nv12Program = SLRRTCCreateProgramFromFragmentSource(kNV12FragmentShaderSource);
  if (!_nv12Program) {
    return NO;
  }
  ...
  if (ySampler < 0 || uvSampler < 0) {
    SLRRTCLog(@"Failed to get uniform variable locations in NV12 shader");
    glDeleteProgram(_nv12Program);
    _nv12Program = 0;
    return NO;
  }
...
- (BOOL)prepareVertexBufferWithRotation:(SLRRTCVideoRotation)rotation {
  if (!_vertexBuffer && !SLRRTCCreateVertexBuffer(&_vertexBuffer, &_vertexArray)) {
    SLRRTCLog(@"Failed to setup vertex buffer");
    return NO;
  }
#if !TARGET_OS_IPHONE
  ...
  if (!_currentRotation || rotation != *_currentRotation) {
    _currentRotation = absl::optional<SLRRTCVideoRotation>(rotation);
    SLRRTCSetVertexData(*_currentRotation);
  }
  return YES;
}
...
- (void)applyShadingForFrameWithWidth:(int)width
  ...
  if (!_i420Program && ![self createAndSetupI420Program]) {
    SLRRTCLog(@"Failed to setup I420 program");
    return;
  }
...
- (void)applyShadingForFrameWithWidth:(int)width
  ...
  if (!_nv12Program && ![self createAndSetupNV12Program]) {
    SLRRTCLog(@"Failed to setup NV12 shader");
    return;
  }
