...
@interface SLRRTCMTLNSVideoView ()<MTKViewDelegate>
@property(nonatomic) id<SLRRTCMTLRenderer> renderer;
@property(nonatomic, strong) MTKView *metalView;
@property(atomic, strong) SLRRTCVideoFrame *videoFrame;
@end

@implementation SLRRTCMTLNSVideoView {
  id<SLRRTCMTLRenderer> _renderer;
}
...
- (void)configure {
  if ([[self class] isMetalAvailable]) {
    ...
    _renderer = [[SLRRTCMTLI420Renderer alloc] init];
    if (![(SLRRTCMTLI420Renderer *)_renderer addRenderingDestination:_metalView]) {
      _renderer = nil;
    };
  }
}
