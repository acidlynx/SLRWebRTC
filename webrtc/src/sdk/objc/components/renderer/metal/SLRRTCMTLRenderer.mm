...
#import "SLRRTCMTLRenderer+Private.h"
...
static NSString *const pipelineDescriptorLabel = @"SLRRTCPipeline";
static NSString *const commandBufferLabel = @"SLRRTCCommandBuffer";
static NSString *const renderEncoderLabel = @"SLRRTCEncoder";
static NSString *const renderEncoderDebugGroup = @"SLRRTCDrawFrame";
...
@implementation SLRRTCMTLRenderer {
  ...
  int _oldCropX;
  int _oldCropY;
  SLRRTCVideoRotation _oldRotation;
}
...
- (void)getWidth:(int *)width
      ...
      cropHeight:(int *)cropHeight
           cropX:(int *)cropX
           cropY:(int *)cropY
         ofFrame:(nonnull SLRRTCVideoFrame *)frame {
  RTC_NOTREACHED() << "Virtual method not implemented in subclass.";
}

- (BOOL)setupTexturesForFrame:(nonnull SLRRTCVideoFrame *)frame {
...
- (BOOL)setupMetal {
  ...
  if (libraryError) {
    SLRRTCLogError(@"Metal: Library with source failed\n%@", libraryError);
    return NO;
  }

  if (!sourceLibrary) {
    SLRRTCLogError(@"Metal: Failed to load library. %@", libraryError);
    return NO;
  }
...
- (void)loadAssets {
  ...
  if (!_pipelineState) {
    SLRRTCLogError(@"Metal: Failed to create pipeline state. %@", error);
  }
}
...
#pragma mark - SLRRTCMTLRenderer

- (void)drawFrame:(SLRRTCVideoFrame *)frame {
  @autoreleasepool {
