...
@implementation SLRRTCMTLNV12Renderer {
  // Textures.
...
- (BOOL)initializeTextureCache {
  ...
  if (status != kCVReturnSuccess) {
    SLRRTCLogError(@"Metal: Failed to initialize metal texture cache. Return status is %d", status);
    return NO;
  }
...
- (void)getWidth:(nonnull int *)width
  ...
  SLRRTCCVPixelBuffer *pixelBuffer = (SLRRTCCVPixelBuffer *)frame.buffer;
...
- (BOOL)setupTexturesForFrame:(nonnull SLRRTCVideoFrame *)frame {
  RTC_DCHECK([frame.buffer isKindOfClass:[SLRRTCCVPixelBuffer class]]);
  if (![super setupTexturesForFrame:frame]) {
    return NO;
  }
  CVPixelBufferRef pixelBuffer = ((SLRRTCCVPixelBuffer *)frame.buffer).pixelBuffer;
