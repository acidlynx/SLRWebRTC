...
#endif
#import "SLRRTCCodecSpecificInfoH264.h"
#import "SLRRTCH264ProfileLevelId.h"
#import "api/peerconnection/SLRRTCRtpFragmentationHeader+Private.h"
#import "api/peerconnection/SLRRTCVideoCodecInfo+Private.h"
#import "base/SLRRTCCodecSpecificInfo.h"
#import "base/SLRRTCI420Buffer.h"
...
@interface SLRRTCVideoEncoderH264 ()

- (void)frameWasEncoded:(OSStatus)status
                  flags:(VTEncodeInfoFlags)infoFlags
           sampleBuffer:(CMSampleBufferRef)sampleBuffer
      codecSpecificInfo:(id<SLRRTCCodecSpecificInfo>)codecSpecificInfo
                  width:(int32_t)width
...
struct SLRRTCFrameEncodeParams {
  SLRRTCFrameEncodeParams(SLRRTCVideoEncoderH264 *e,
                       SLRRTCCodecSpecificInfoH264 *csi,
                       int32_t w,
                       int32_t h,
                       int64_t rtms,
                       uint32_t ts,
                       SLRRTCVideoRotation r)
      : encoder(e), width(w), height(h), render_time_ms(rtms), timestamp(ts), rotation(r) {
    if (csi) {
      codecSpecificInfo = csi;
    } else {
      codecSpecificInfo = [[SLRRTCCodecSpecificInfoH264 alloc] init];
    }
  }

  SLRRTCVideoEncoderH264 *encoder;
  SLRRTCCodecSpecificInfoH264 *codecSpecificInfo;
  int32_t width;
  int32_t height;
...
bool CopyVideoFrameToNV12PixelBuffer(id<SLRRTCI420Buffer> frameBuffer, CVPixelBufferRef pixelBuffer) {
...
void compressionOutputCallback(void *encoder,
  ...
  std::unique_ptr<SLRRTCFrameEncodeParams> encodeParams(
      reinterpret_cast<SLRRTCFrameEncodeParams *>(params));
...
@implementation SLRRTCVideoEncoderH264 {
  SLRRTCVideoCodecInfo *_codecInfo;
  std::unique_ptr<webrtc::BitrateAdjuster> _bitrateAdjuster;
  uint32_t _targetBitrateBps;
  uint32_t _encoderBitrateBps;
  uint32_t _encoderFrameRate;
  uint32_t _maxAllowedFrameRate;
  SLRRTCH264PacketizationMode _packetizationMode;
  absl::optional<webrtc::H264::ProfileLevelId> _profile_level_id;
  SLRRTCVideoEncoderCallback _callback;
  int32_t _width;
  int32_t _height;
  VTCompressionSessionRef _compressionSession;
  CVPixelBufferPoolRef _pixelBufferPool;
  SLRRTCVideoCodecMode _mode;
...
- (instancetype)initWithCodecInfo:(SLRRTCVideoCodecInfo *)codecInfo {
  if (self = [super init]) {
    ...
    _packetizationMode = SLRRTCH264PacketizationModeNonInterleaved;
    _profile_level_id =
...
- (NSInteger)encode:(SLRRTCVideoFrame *)frame
    codecSpecificInfo:(nullable id<SLRRTCCodecSpecificInfo>)codecSpecificInfo
  ...
  if ([frame.buffer isKindOfClass:[SLRRTCCVPixelBuffer class]]) {
    // Native frame buffer
    SLRRTCCVPixelBuffer *rtcPixelBuffer = (SLRRTCCVPixelBuffer *)frame.buffer;
  ...
  if (!isKeyframeRequired && frameTypes) {
    for (NSNumber *frameType in frameTypes) {
      if ((SLRRTCFrameType)frameType.intValue == SLRRTCFrameTypeVideoFrameKey) {
        isKeyframeRequired = YES;
        break;
      }
    }
  ...
  std::unique_ptr<SLRRTCFrameEncodeParams> encodeParams;
  encodeParams.reset(new SLRRTCFrameEncodeParams(self,
                                              codecSpecificInfo,
                                              _width,
                                              _height,
...
- (OSType)pixelFormatOfFrame:(SLRRTCVideoFrame *)frame {
...
- (BOOL)resetCompressionSessionIfNeededWithFrame:(SLRRTCVideoFrame *)frame {
...
- (void)frameWasEncoded:(OSStatus)status
  ...
  SLRRTCRtpFragmentationHeader *header;
  {
    ...
    header = [[SLRRTCRtpFragmentationHeader alloc] initWithNativeFragmentationHeader:header_cpp.get()];
    if (!result) {
      return;
    }
  }

  SLRRTCEncodedImage *frame = [[SLRRTCEncodedImage alloc] init];
  frame.buffer = [NSData dataWithBytesNoCopy:buffer->data() length:buffer->size() freeWhenDone:NO];
  frame.encodedWidth = width;
  frame.encodedHeight = height;
  frame.completeFrame = YES;
  frame.frameType = isKeyframe ? SLRRTCFrameTypeVideoFrameKey : SLRRTCFrameTypeVideoFrameDelta;
  frame.captureTimeMs = renderTimeMs;
  frame.timeStamp = timestamp;
  frame.rotation = rotation;
  frame.contentType = (_mode == SLRRTCVideoCodecModeScreensharing) ? SLRRTCVideoContentTypeScreenshare :
...
- (nullable SLRRTCVideoEncoderQpThresholds *)scalingSettings {
  return [[SLRRTCVideoEncoderQpThresholds alloc] initWithThresholdsLow:kLowH264QpThreshold
