...
@interface SLRRTCH264ProfileLevelId ()

@property(nonatomic, assign) SLRRTCH264Profile profile;
@property(nonatomic, assign) SLRRTCH264Level level;
@property(nonatomic, strong) NSString *hexString;

@end

@implementation SLRRTCH264ProfileLevelId
...
- (instancetype)initWithHexString:(NSString *)hexString {
  if (self = [super init]) {
    ...
    if (profile_level_id.has_value()) {
      self.profile = static_cast<SLRRTCH264Profile>(profile_level_id->profile);
      self.level = static_cast<SLRRTCH264Level>(profile_level_id->level);
    }
  }
  return self;
}
