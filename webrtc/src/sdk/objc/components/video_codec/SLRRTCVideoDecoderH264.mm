...
struct SLRRTCFrameDecodeParams {
  SLRRTCFrameDecodeParams(SLRRTCVideoDecoderCallback cb, int64_t ts) : callback(cb), timestamp(ts) {}
  SLRRTCVideoDecoderCallback callback;
  int64_t timestamp;
};

@interface SLRRTCVideoDecoderH264 ()
...
void decompressionOutputCallback(void *decoderRef,
                                 ...
                                 CMTime timestamp,
                                 CMTime duration) {
  std::unique_ptr<SLRRTCFrameDecodeParams> decodeParams(
      reinterpret_cast<SLRRTCFrameDecodeParams *>(params));
  if (status != noErr) {
    SLRRTCVideoDecoderH264 *decoder = (__bridge SLRRTCVideoDecoderH264 *)decoderRef;
    [decoder setError:status];
    RTC_LOG(LS_ERROR) << "Failed to decode frame. Status: " << status;
    return;
  }
  // TODO(tkchin): Handle CVO properly.
  SLRRTCCVPixelBuffer *frameBuffer = [[SLRRTCCVPixelBuffer alloc] initWithPixelBuffer:imageBuffer];
  SLRRTCVideoFrame *decodedFrame =
      [[SLRRTCVideoFrame alloc] initWithBuffer:frameBuffer
...
// Decoder.
@implementation SLRRTCVideoDecoderH264 {
  CMVideoFormatDescriptionRef _videoFormat;
  CMMemoryPoolRef _memoryPool;
  VTDecompressionSessionRef _decompressionSession;
  SLRRTCVideoDecoderCallback _callback;
  OSStatus _error;
}
...
- (NSInteger)decode:(SLRRTCEncodedImage *)inputImage
  ...
  std::unique_ptr<SLRRTCFrameDecodeParams> frameDecodeParams;
  frameDecodeParams.reset(new SLRRTCFrameDecodeParams(_callback, inputImage.timeStamp));
