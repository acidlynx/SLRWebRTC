...
#import "SLRRTCEncodedImage+Private.h"

#include "rtc_base/numerics/safe_conversions.h"

@implementation SLRRTCEncodedImage (Private)
...
- (instancetype)initWithNativeEncodedImage:(const webrtc::EncodedImage &)encodedImage {
  if (self = [super init]) {
    ...
    self.frameType = static_cast<SLRRTCFrameType>(encodedImage._frameType);
    self.rotation = static_cast<SLRRTCVideoRotation>(encodedImage.rotation_);
...
- (webrtc::EncodedImage)nativeEncodedImage {
  ...
  encodedImage.content_type_ = (self.contentType == SLRRTCVideoContentTypeScreenshare) ?
