...
void SLRRTCInitFieldTrialDictionary(NSDictionary<NSString *, NSString *> *fieldTrials) {
  if (!fieldTrials) {
    SLRRTCLogWarning(@"No fieldTrials provided.");
    return;
  }
  ...
  if (![fieldTrialInitString getCString:gFieldTrialInitString.get()
    ...
    SLRRTCLogError(@"Failed to convert field trial string.");
    return;
  }
