...
@implementation SLRRTCStatistics
...
@implementation SLRRTCStatisticsReport
...
@implementation SLRRTCStatisticsReport (Private)
...
- (instancetype)initWithReport:(const webrtc::RTCStatsReport &)report {
  if (self = [super init]) {
    ...
    for (const auto &stat : report) {
      SLRRTCStatistics *statistics = [[SLRRTCStatistics alloc] initWithStatistics:stat];
