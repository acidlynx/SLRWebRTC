...
#import "SLRRTCRtcpParameters+Private.h"
#import "SLRRTCRtpCodecParameters+Private.h"
#import "SLRRTCRtpEncodingParameters+Private.h"
#import "SLRRTCRtpHeaderExtension+Private.h"
#import "helpers/NSString+StdString.h"

@implementation SLRRTCRtpParameters
...
- (instancetype)initWithNativeParameters:
  ...
  if (self = [self init]) {
    ...
    _rtcp = [[SLRRTCRtcpParameters alloc] initWithNativeParameters:nativeParameters.rtcp];
    ...
    for (const auto &headerExtension : nativeParameters.header_extensions) {
      [headerExtensions
          addObject:[[SLRRTCRtpHeaderExtension alloc] initWithNativeParameters:headerExtension]];
    }
    _headerExtensions = headerExtensions;

    NSMutableArray *encodings = [[NSMutableArray alloc] init];
    for (const auto &encoding : nativeParameters.encodings) {
      [encodings addObject:[[SLRRTCRtpEncodingParameters alloc]
    ...
    for (const auto &codec : nativeParameters.codecs) {
      [codecs addObject:[[SLRRTCRtpCodecParameters alloc]
...
- (webrtc::RtpParameters)nativeParameters {
  ...
  for (SLRRTCRtpHeaderExtension *headerExtension in _headerExtensions) {
    parameters.header_extensions.push_back(headerExtension.nativeParameters);
  }
  for (SLRRTCRtpEncodingParameters *encoding in _encodings) {
    parameters.encodings.push_back(encoding.nativeParameters);
  }
  for (SLRRTCRtpCodecParameters *codec in _codecs) {
