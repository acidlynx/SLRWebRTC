...
#import "SLRRTCPeerConnectionFactoryOptions+Private.h"
...
@implementation SLRRTCPeerConnectionFactory {
...
- (instancetype)init {
  ...
  return [self initWithNativeAudioEncoderFactory:webrtc::CreateBuiltinAudioEncoderFactory()
                       ...
                       nativeVideoEncoderFactory:webrtc::ObjCToNativeVideoEncoderFactory(
                                                     [[SLRRTCVideoEncoderFactoryH264 alloc] init])
                       nativeVideoDecoderFactory:webrtc::ObjCToNativeVideoDecoderFactory(
                                                     [[SLRRTCVideoDecoderFactoryH264 alloc] init])
...
- (SLRRTCAudioSource *)audioSourceWithConstraints:(nullable SLRRTCMediaConstraints *)constraints {
  ...
  return [[SLRRTCAudioSource alloc] initWithFactory:self nativeAudioSource:source];
}

- (SLRRTCAudioTrack *)audioTrackWithTrackId:(NSString *)trackId {
  SLRRTCAudioSource *audioSource = [self audioSourceWithConstraints:nil];
  return [self audioTrackWithSource:audioSource trackId:trackId];
}

- (SLRRTCAudioTrack *)audioTrackWithSource:(SLRRTCAudioSource *)source
                                trackId:(NSString *)trackId {
  return [[SLRRTCAudioTrack alloc] initWithFactory:self
                                         source:source
                                        trackId:trackId];
}

- (SLRRTCVideoSource *)videoSource {
  return [[SLRRTCVideoSource alloc] initWithFactory:self
                                 signalingThread:_signalingThread.get()
                                    workerThread:_workerThread.get()];
}

- (SLRRTCVideoTrack *)videoTrackWithSource:(SLRRTCVideoSource *)source
                                trackId:(NSString *)trackId {
  return [[SLRRTCVideoTrack alloc] initWithFactory:self
                                         source:source
                                        trackId:trackId];
}

- (SLRRTCMediaStream *)mediaStreamWithStreamId:(NSString *)streamId {
  return [[SLRRTCMediaStream alloc] initWithFactory:self
                                        streamId:streamId];
}

- (SLRRTCPeerConnection *)peerConnectionWithConfiguration:
    (SLRRTCConfiguration *)configuration
                                           constraints:
    (SLRRTCMediaConstraints *)constraints
                                              delegate:
    (nullable id<SLRRTCPeerConnectionDelegate>)delegate {
  return [[SLRRTCPeerConnection alloc] initWithFactory:self
...
- (BOOL)startAecDumpWithFilePath:(NSString *)filePath
  ...
  if (_hasStartedAecDump) {
    SLRRTCLogError(@"Aec dump already started.");
    return NO;
  }
