...
SDK_FRAMEWORK_NAME = 'SLRWebRTC.framework'
...
def main():
  ...
  # Combine the slices.
  dylib_path = os.path.join(SDK_FRAMEWORK_NAME, 'SLRWebRTC')
  ...
  lib_dsym_dir_path = os.path.join(lib_paths[0], 'SLRWebRTC.dSYM')
  if os.path.isdir(lib_dsym_dir_path):
    distutils.dir_util.copy_tree(lib_dsym_dir_path,
                                  os.path.join(args.output_dir, 'SLRWebRTC.dSYM'))
    logging.info('Merging dSYM slices.')
    dsym_path = os.path.join('SLRWebRTC.dSYM', 'Contents', 'Resources', 'DWARF',
                              'SLRWebRTC')
