...
class ReportBlockStatsTest : public ::testing::Test {
 protected:
  ...
  RtcpStatistics RtcpReportBlockToRtcpStatistics(const SLRRTCPReportBlock& stats) {
  ...
  SLRRTCPReportBlock block1_1_;
  SLRRTCPReportBlock block1_2_;
  SLRRTCPReportBlock block1_3_;
  SLRRTCPReportBlock block2_1_;
  SLRRTCPReportBlock block2_2_;
  std::vector<SLRRTCPReportBlock> ssrc1block1_;
  std::vector<SLRRTCPReportBlock> ssrc1block2_;
  std::vector<SLRRTCPReportBlock> ssrc12block1_;
  std::vector<SLRRTCPReportBlock> ssrc12block2_;
};

TEST_F(ReportBlockStatsTest, AggregateAndStore_NoSsrc) {
  ReportBlockStats stats;
  std::vector<SLRRTCPReportBlock> empty;
  SLRRTCPReportBlock aggregated = stats.AggregateAndStore(empty);
...
TEST_F(ReportBlockStatsTest, AggregateAndStore_OneSsrc) {
  ReportBlockStats stats;
  SLRRTCPReportBlock aggregated = stats.AggregateAndStore(ssrc1block1_);
...
TEST_F(ReportBlockStatsTest, AggregateAndStore_TwoSsrcs) {
  ReportBlockStats stats;
  SLRRTCPReportBlock aggregated = stats.AggregateAndStore(ssrc12block1_);
  EXPECT_EQ(0U, aggregated.fraction_lost);
