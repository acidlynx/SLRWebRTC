...
class ReportBlockStats {
 public:
  typedef std::map<uint32_t, SLRRTCPReportBlock> ReportBlockMap;
  typedef std::vector<SLRRTCPReportBlock> ReportBlockVector;
  ReportBlockStats();
  ~ReportBlockStats();

  // Updates stats and stores report blocks.
  // Returns an aggregate of the |report_blocks|.
  SLRRTCPReportBlock AggregateAndStore(const ReportBlockVector& report_blocks);
 ...
 private:
  ...
  void StoreAndAddPacketIncrement(const SLRRTCPReportBlock& report_block,
